...
SRC = Path('/Users/apple/temp/DH Social Media Metrics FB Data.csv')
...
def clean_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'media_type' not in out.columns:
        video_views = pd.to_numeric(out.get('post_video_views'), errors='coerce').fillna(0)
...
def add_charts_to_payload(payload: dict) -> dict:
    ...
    fig_genre.update_layout(title=f'Facebook {lang.upper()}: Genre Median Reach')
    ...
    fig_pareto.update_layout(title=f'Facebook {lang.upper()}: Pareto by Genre')
    ...
    fig_combo_median.update_layout(title=f'Facebook {lang.upper()}: Genre x Emotion by Median Reach')
    ...
    fig_combo_share.update_layout(title=f'Facebook {lang.upper()}: Genre x Emotion by Reach Share')
...
<head>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; }}
        ...
        .metric-card {{ display: inline-block; min-width: 240px; border: 1px solid #cfd9e5; border-radius: 12px; background: linear-gradient(135deg, #eaf7f4, #eef4ff); padding: 8px 12px; }}
        .metric-k {{ font-size: 10px; color: #385269; text-transform: uppercase; }}
        .metric-v {{ font-size: 20px; font-weight: 700; color: #1c3347; margin-top: 2px; }}
